...
> **0920 release update:** MCP Server 1.0.12 uses one identical MJS file for Chrome and Firefox, supports a standby browser connection, improves tool-schema compatibility and pagination, and retains all 51 tools. Fixes port-conflict shutdown, lingering processes after stdio EOF, Firefox navigation cancellation signals, and native-input receipts. browser_read_text now returns text consistently in structured value and reports missing payloads explicitly. Click, type, select, key and wait calls return concise receipts while retaining outcome and input evidence; include_elements opts into page details. Evaluate omits console logs unless include_logs is enabled; navigation provides a bounded preview with refs unless include_snapshot is enabled. Large results continue through next_page_token. Fixes reused refs across SPA routes, title-based outcome misclassification and repeated find contexts. Proxy routing fixes default-port validation, invalid-address application and browser-proxy discovery; Firefox uses native request routing. Unmatched requests go direct in Chrome and retain the browser default route in Firefox. Page scripts fix missed reinjection after reload, stale-code execution after failed saves, duplicate injection, execution of disabled scripts and incorrect target tabs; registration is serialized and save results checked. Agent repair after truncated decisions increases the output budget and disables thinking for that repair; persistent exhaustion shows clear pause and recovery guidance. Upgrade and reload the extension, replace the MJS file, and restart the MCP service.
